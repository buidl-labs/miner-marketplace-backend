...
func (r *minerResolver) FinanceMetrics(ctx context.Context, obj *model.Miner, since *int, till *int) (*model.FinanceMetrics, error) {
	...
	/*
		...
		if ownerDidChange {
			...
			err = r.DB.Model(&txns).Column("amount", "miner_tip", "base_fee_burn", "transferred").
				Where("actor_name = ? AND method = ? AND sender = ?", "fil/3/storagemarket", 3, mi.OwnerID).
		...
		if workerDidChange {
			...
			var transfers []string
			err := r.DB.Model(&txns).Column("amount", "miner_tip", "base_fee_burn", "transferred").
				Where("sender = ? AND actor_name = ? AND method != ?", mi.WorkerID, "fil/3/storageminer", 5).
...
func (r *minerResolver) Transactions(ctx context.Context, obj *model.Miner, since *int, till *int) ([]*model.Transaction, error) {
	...
	// 	panic(err)
	// }
	setIncoming := make(map[string]int)
	setOutgoing := make(map[string]int)

	fmt.Println("LENS", len(incoming), len(outgoing))
	var transactions []*model.Transaction
	for _, txn := range incoming {
		if _, ok := setIncoming[txn.Cid]; ok {
			fmt.Println("element found")
		} else {
			fmt.Println("element not found")
			setIncoming[txn.Cid] = 1
			amt := txn.Amount
			if txn.Amount == "0" {
				if txn.Transferred != "0" {
					amt = txn.Transferred
	...
	for _, txn := range outgoing {
		if _, ok := setOutgoing[txn.Cid]; ok {
			fmt.Println("element found")
		} else {
			fmt.Println("element not found")
			setIncoming[txn.Cid] = 1
